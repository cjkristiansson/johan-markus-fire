...
<style>
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .scenario-card { background: #161616; border: 1px solid #262626; }
...
def simulate(name, boligpris, udbet_j, udbet_m, realkredit, ejerudgift, bolig_solgt):
    ...
    for y in range(0, 26):
        ...
        h_j = max(0, cur_exp_j - p_j) / (global_barista_wage_net * 4.33)
        h_m = max(0, cur_exp_m - p_m) / (global_barista_wage_net * 4.33)
    ...
    st.markdown("<div class='success-box'>✅ Brofinansiering simuleret succesfuldt.</div>", unsafe_allow_html=True)
